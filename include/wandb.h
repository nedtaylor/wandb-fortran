 ...
 *
 * Calls wandb.finish() and releases the run reference, but keeps the
 * Python interpreter alive so that wandb_init_c can be called again
 * (e.g., for sweep runs or sequential runs).
 */
 ...
 *
 * Call once, after all wandb runs (including sweep runs) are done.
 * No wandb functions should be called after this.
 */
